...
# file_list=    "2655/sampleSelection/1/summsize/TopMAD5000/umapKNN_set.2655.TopMAD5000.list"
# dataset_label="2655"
# dir_out=      "test/"
# cutoff_diag_n= 5
...
df_filter_list=df_out %>% filter(!diag == KNN_pred) %>% group_by(COH_sample) %>% mutate(
  FeatureN_failed=paste0(FeatureN,collapse = ","),NegihborN_failed=paste0(n_neighbors,collapse = ",")
...
# table(df_out1$diag)
output_base=paste0(dir_out,"umapKNN_set.",dataset_label)
