...
determineSoftPowerWGCNA <- function(data1, outFile, propGenes=1){
        ...
        sft <- pickSoftThreshold(t(data1), powerVector=powers, verbose=5,networkType = "signed")
        # Plot the results:
        png(outFile, width=250, height=125, units="mm", res=300)
        par(mfrow = c(1,2))
        cex1 <- 0.9
        
        # Scale-free topology fit index as a function of the soft-thresholding power
        y_value=-sign(sft$fitIndices[,3])*sft$fitIndices[,2]
        cutoff=round(y_value[sft$fitIndices$Power==sft$powerEstimate],2)
                print(paste0('Estimate_power=',sft$powerEstimate,"; Cutoff=",cutoff))
...
runWGCNA <- function(data1, propGenes, softPower=10, signedNetwork=TRUE,corFun_tmp="bicor"){
        ...
        datExprA1g <- data1
        adjacencyA1 <- adjacency(t(datExprA1g),power=softPower,type=type,corFnc = corFun_tmp)
        diag(adjacencyA1) <- 0
...
plotModulesCut <- function(referenceDataset, outFile, minClusterSize=30){
        ...
        plotDendroAndColors(referenceDataset[["geneTreeA1"]], mColorh,rowTextAlignment = c("right"),
